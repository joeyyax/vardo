...
// ---------------------------------------------------------------------------
// These tests cover two security/correctness paths added in the GPU feature:
//
//   1. Role gate in PATCH /apps/[appId]: only owner and admin may set
//      gpuEnabled=true. Members must receive 403.
//
//   2. Rollback config restore: gpuEnabled is correctly restored from a
//      configSnapshot, with the ?? false fallback for legacy snapshots that
//      predate the field.
//
// Both are tested as extracted pure functions so no database or Next.js
...
// ---------------------------------------------------------------------------
// 2. Rollback config restore logic (mirrors rollback/route.ts)
// ---------------------------------------------------------------------------
