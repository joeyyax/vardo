...
import { projects, memberships, githubAppInstallations } from "@/lib/db/schema";
...
// GET /api/v1/organizations/[orgId]/projects/[projectId]/branches
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const httpsMatch = project.gitUrl.match(/github\.com\/([\w.-]+)\/([\w.-]+?)(?:\.git)?$/);
    if (httpsMatch) {
      owner = httpsMatch[1];
      repo = httpsMatch[2];
    }
    ...
    if (!owner) {
      const sshMatch = project.gitUrl.match(/github\.com:([\w.-]+)\/([\w.-]+?)(?:\.git)?$/);
      if (sshMatch) {
        owner = sshMatch[1];
        repo = sshMatch[2];
      }
