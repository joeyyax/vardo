...
import { clientComments, clients } from "@/lib/db/schema";
import { requireOrg } from "@/lib/auth/session";
import { eq, and } from "drizzle-orm";

type RouteParams = {
  params: Promise<{ orgId: string; clientId: string; commentId: string }>;
};
...
// PATCH /api/v1/organizations/[orgId]/clients/[clientId]/comments/[commentId]
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    const { orgId, clientId, commentId } = await params;
    ...
    const existingComment = await db.query.clientComments.findFirst({
      where: and(
        eq(clientComments.id, commentId),
    ...
    const fullComment = await db.query.clientComments.findFirst({
      where: eq(clientComments.id, updatedComment.id),
      with: {
        author: {
...
// DELETE /api/v1/organizations/[orgId]/clients/[clientId]/comments/[commentId]
